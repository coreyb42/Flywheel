 ...
 *
 * <p>Legacy instancing supplied this data through {@code TextureBuffer}. In
 * 26.2 it is a second vertex binding instead. The stride returned from
 * {@link #upload(InstancedInstancer)} must match the instanced vertex binding
...
public final class GpuInstanceBufferAdapter implements AutoCloseable {
	...
	/**
	 * Upload a synchronized snapshot of {@code instancer}'s current instances.
	 * This must run before opening the render pass that consumes the result.
	 */
	public <I extends Instance> Snapshot upload(InstancedInstancer<I> instancer) {
		ensureOpen();
		Objects.requireNonNull(instancer, "instancer");
		int stride = instancer.instanceStride();
		int expectedCount = instancer.instanceCount();
		long byteCount = Math.multiplyExact((long) stride, expectedCount);
		if (byteCount == 0) {
			return new Snapshot(0, stride);
		}
		if (byteCount > Integer.MAX_VALUE) {
			throw new IllegalArgumentException("Instance upload is too large for a CPU buffer");
		}
